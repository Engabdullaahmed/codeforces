...
fashionable if min(a)+max(a) is divisible by 2 without a remainder, where min(a) — the value of the minimum element of the array a, and max(a) — the value of the maximum element of the array a
...
(1≤n≤50) — the size of the array a
...
, and 7 is not divisible by 2. If any of the elements are removed, only one element will remain. Then max(a)+min(a) will be divisible by 2

.

In the second test case, nothing needs to be removed since min(a)+max(a)=1+9=10
, and 10 is divisible by 2

.

In the third test case, you can remove the elements with values 2
and 4, then min(a)+max(a)=5+11=16, and 16 is divisible by 2
...
int main ()
{
    ...
    while (t--)
    {
        ...
        // at this part we compare with big number 
        int ans = 0 ;
        for(int i = 0 ; i < n ; i++)
        {
            if((a[i]+a[n-1])%2==0)
        ...
        // at this part we compare with small number 
        int ans2 = 0;
        for(int i = n-1 ; i >= 0 ; i--)
        {
            if((a[i]+a[0])%2==0)
