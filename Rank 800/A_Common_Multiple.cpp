...
. An array x1,x2,…,xm is beautiful if there exists an array y1,y2,…,ym such that the elements of y are distinct (in other words, yi≠yj for all 1≤i<j≤m), and the product of xi and yi is the same for all 1≤i≤m (in other words, xi⋅yi=xj⋅yj for all 1≤i<j≤m
...
is already beautiful. A possible array y is [6,3,2], which is valid since the elements of y are distinct, and 1⋅6=2⋅3=3⋅2

.

In the second test case, the subsequence [3,1,4,5]
is beautiful. A possible array y is [20,60,15,12]. It can be proven that the entire array a=[3,1,4,1,5] is not beautiful, so the maximum size of a subsequence of array a that is beautiful is 4
