...
A string r
consisting only of lowercase Latin letters is called universal if and only if r is lexicographically smaller∗ than the reversal† of r

.

You are given a string s
consisting of n lowercase Latin letters. You are required to make s universal. To achieve this, you can perform the following operation on s at most k

times:

    Choose two indices i

and j (1≤i,j≤n), then swap si and sj. Note that if i=j
...
will keep the same after any operations. However, the reversal of a is still a, so it is impossible to make s

universal.

In the second test case, the string rev
is lexicographically smaller than ver. Thus, s
...
and s7. After this operation, s becomes uniserval
;
Swap s1
and s3. After this operation, s becomes inuserval
